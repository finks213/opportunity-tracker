...
- 2 automated gate(s) FAIL: fullSuite, reportIntegrity
- 33 automated gate(s) UNVERIFIED: fixtureIntegrity, fixtureRoundTrip, deterministicHydration, pairedWorlds, exactProbability, matchedTrajectory, meaningfulTraits, birthImmutability, observerInvariance, noObserverDeps, neutralTraits, mutationIndependence, mutationProvenance, allocationOpportunity, spatialIntegrity, lifecycle, genealogyIntegrity, genealogyBounded, survivalComposition, rngIntegrity, populationGuardrails, traversal, desktopMeasurement, modelIdentity, modelHashProvenance, legibilitySelfContained, worldLoadRaceSafety, observerTransaction, generationResultImmutable, focalLineage, referencesUnchanged, testTreeIntegrity, reportDeterminism
- 5 failing test(s) map to no declared gate
...
|---|---|---|---|---|
| 1 | Full test suite | §20 | `audit/test-results.txt` (TAP summary) | **FAIL** — 391/398, 7 failing |
...
| 34 | Report agrees with the raw evidence | §26 / §27 | 4 named test(s) | **FAIL** — failing: §26 — the committed report is exactly what the generator produces; §20 — the report's suite counts equal the raw TAP summary |
...
**Gate totals:** of 38 gates, 3 are externally determined and 35 are machine-verified from named tests. Machine-verified: 0 PASS · 2 FAIL · 33 UNVERIFIED. Externally determined: `desktopCanvasMemory` UNVERIFIED · `ipadGate` PENDING_HUMAN_DEVICE_TEST · `stageAOrder` VIOLATED — principal decision required.

The four numbers 0 + 2 + 33 + 3 sum to 38, which is the denominator 38. No gate is counted in two categories.

**Unattributed failures present.** The run contains failing tests that no gate claims, so every gate not directly evidenced by a passing test is reported `UNVERIFIED` rather than keeping a prior PASS: §26 — rendering is a pure function of committed evidence; §26/§28 — a failing run regenerates a report whose gate, totals and STATUS all change; §20/§26 — the committed report and gate summary agree with a fresh derivation; §26 — the report's status is the one the evidence derives; §20/§26 — the committed gate summary matches the report's table
...
| full test suite from clean | 391/398, 7 failures |
...
| every automated result reproducible from a clean run | NOT met — 391/398 from clean, 2 gate(s) FAIL, 33 UNVERIFIED |
