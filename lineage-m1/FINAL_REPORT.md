...
- 1 failing test(s) map to no declared gate
...
|---|---|---|---|---|
| 1 | Full test suite | §20 | `audit/test-results.txt` (TAP summary) | **FAIL** — 397/398, 1 failing |
...
**Unattributed failures present.** The run contains failing tests that no gate claims, so every gate not directly evidenced by a passing test is reported `UNVERIFIED` rather than keeping a prior PASS: §20 — the committed tree-integrity record is a VALID proof, not merely a run
...
| full test suite from clean | 397/398, 1 failures |
...
| every automated result reproducible from a clean run | NOT met — 397/398 from clean, 1 gate(s) FAIL, 34 UNVERIFIED |
